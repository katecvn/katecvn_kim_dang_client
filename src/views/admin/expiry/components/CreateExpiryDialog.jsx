...
import { createExpiry, getExpiriesByCustomerId } from '@/stores/ExpirySlice'
import { Textarea } from '@/components/ui/textarea'
import { getProducts } from '@/stores/ProductSlice'
import { AvatarImage, AvatarFallback, Avatar } from '@/components/ui/avatar'
import { CheckIcon, Mail, MapPin, X } from 'lucide-react'
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
import { createExpirySchema } from '../schema'

const CreateExpiryDialog = ({
  open,
  onOpenChange,
  showTrigger = true,
  ...props
  ...
  const form = useForm({
    resolver: zodResolver(createExpirySchema),
    defaultValues: {
      name: '',
      customerId: 0,
      productId: 0,
      ...
      alertDateStep: 30,
      note: '',
      userId: authUserWithRoleHasPermissions.id || 0,
      accountId: 0,
    },
  })
  ...
  // Khi nhập tên tài khoản
  const handleAccountNameChange = (name) => {
    const normalized = (s) => (s || '').trim().toLowerCase()
    const matched = customerAccounts.find(
      (acc) => normalized(acc.accountName) === normalized(name),
    )

    if (matched) {
      form.setValue('accountId', matched?.id)
      form.setValue('productId', matched.expiries?.[0]?.productId || 0)
      form.setValue('name', matched.accountName, { shouldValidate: true })
      setSelectedAccount(matched)
      setSelectedProduct(
        products.find((p) => p.id === matched.expiries?.[0]?.productId) || null,
      )
      setNearestExpiryDate(matched.expiries?.[0]?.endDate || null)
    } else {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await dispatch(createExpiry(payload)).unwrap()
      form.reset()
      setCustomerAccounts([])
    ...
    <div>
      <Dialog open={open} onOpenChange={onOpenChange} {...props}>
        {showTrigger && (
          <DialogTrigger>
            <p className="cursor-pointer font-bold text-primary">
              + Tạo thông tin quản lý hạn dùng
            </p>
          </DialogTrigger>
        )}
        ...
        <DialogContent className="md:h-auto md:max-w-4xl">
          <DialogHeader>
            <DialogTitle>Tạo thông tin quản lý hạn dùng</DialogTitle>
            <DialogDescription>
              Điền thông tin bên dưới để tạo mới một thông tin quản lý hạn dùng.
              <br />
              <span className="text-red-500">Lưu ý: </span>
              <span className="text-blue-500">
                Nếu muốn tạo mới tài khoản thì không chọn tài khoản khách hàng
              </span>
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="max-h-[65vh] overflow-auto md:max-h-[75vh]">
            <Form {...form}>
              <form id="create-expiry" onSubmit={form.handleSubmit(onSubmit)}>
                          ...
                          <FormItem>
                            ...
                            <FormControl>
                              <div>
                                ...
                                />
                                <datalist id="accountNameSuggestions">
          ...
          <DialogFooter className="flex gap-2 sm:space-x-0">
            ...
            <Button form="create-expiry" loading={loading}>
              Tạo mới
            </Button>
          </DialogFooter>
        </DialogContent>
...
export default CreateExpiryDialog
