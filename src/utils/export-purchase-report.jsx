...
export const exportPurchaseReportToExcel = async (data, dateRange) => {
  try {
    const workbook = new ExcelJS.Workbook()
    const worksheet = workbook.addWorksheet('Báo Cáo Tiền Mua')
    ...
    worksheet.getCell('A4').value = 'BÁO CÁO TIỀN MUA'
    ...
    const columns = [
      ...
      { header: 'Số đơn mua', key: 'orderCount', width: 15 },
      { header: 'Tiền mua', key: 'totalPurchase', width: 20 },
    ...
    worksheet.getCell(`D${currentRow}`).value = totals.totalPurchase
    ...
    if (data && data.length > 0) {
      data.forEach((item, index) => {
        ...
        row.getCell(4).value = totalPurchase
    ...
    const fileName = `Bao_Cao_Tien_Mua_${new Date().getTime()}.xlsx`
