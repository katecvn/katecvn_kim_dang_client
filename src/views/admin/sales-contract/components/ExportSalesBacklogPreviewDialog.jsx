...
import { exportSalesBacklogToExcel } from '@/utils/export-sales-backlog'
...
const ExportSalesBacklogPreviewDialog = ({
  ...
  const handleExport = () => {
    if (data) {
      exportSalesBacklogToExcel(data)
      onOpenChange(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn("md:max-w-[95vw] max-h-[90vh] flex flex-col", contentClassName)} overlayClassName={overlayClassName}>
        <DialogHeader>
          <DialogTitle>Xem trước danh sách đơn bán chưa giao</DialogTitle>
        </DialogHeader>
          ...
          <Table className="relative w-full">
            <TableHeader className="sticky top-0 bg-secondary z-10">
              <TableRow>
                ...
                <TableHead className="min-w-[150px] border-r">Khách hàng</TableHead>
                <TableHead className="min-w-[150px] border-r">Sản phẩm</TableHead>
                <TableHead className="text-center border-r">SL Đặt</TableHead>
                <TableHead className="text-center border-r">SL Giao</TableHead>
              ...
              {flattenedData.map((item, index) => {
                ...
                return (
                  <TableRow key={index}>
                    ...
                    <TableCell className="border-r text-center">{Number(item.deliveredQuantity) || 0}</TableCell>
...
export default ExportSalesBacklogPreviewDialog
