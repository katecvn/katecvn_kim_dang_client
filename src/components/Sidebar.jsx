...
const Sidebar = ({ className, isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <aside
      className={cn(
        `fixed left-0 right-0 top-0 z-[500] w-full border-r-2 border-r-muted transition-[width] md:z-50 md:bottom-0 md:right-auto md:h-svh ${isCollapsed ? 'md:w-14' : 'md:w-64'}`,
        className,
      )}
    >
      {/* Overlay in mobile */}
      ...
      <Layout>
        {/* Header */}
        <LayoutHeader className="sticky top-0 justify-between bg-primary px-4 py-3 shadow dark:bg-secondary md:px-4">
          <Link to={'/dashboard'}>
