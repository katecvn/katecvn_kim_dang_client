...
import { exportRevenueToExcel } from '@/utils/export-revenue'
import { moneyFormat } from '@/utils/money-format'
import { IconDownload } from '@tabler/icons-react'
import { cn } from '@/lib/utils'
import { useMemo } from 'react'

const ExportRevenuePreviewDialog = ({
  open,
  onOpenChange,
  data,
  filters,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn("md:max-w-[80vw] max-h-[90vh] flex flex-col", contentClassName)} overlayClassName={overlayClassName}>
        <DialogHeader>
          <DialogTitle>Xem trước báo cáo doanh thu</DialogTitle>
        </DialogHeader>
          ...
          <Table className="relative w-full">
            <TableHeader className="sticky top-0 bg-secondary z-10">
              <TableRow>
                ...
                <TableHead className="w-[100px] border-r text-center">Số đơn hàng</TableHead>
                <TableHead className="border-r text-right">Doanh số</TableHead>
            ...
            <TableBody>
              ...
              <TableRow className="font-bold bg-muted/50 text-red-600 sticky top-[calc(theme(spacing.10))] z-10 shadow-sm">
                ...
                <TableCell className="border-r text-right">{moneyFormat(totals.totalSales)}</TableCell>
              ...
              {data?.map((item, index) => {
                ...
                return (
                  <TableRow key={index}>
                    ...
                    <TableCell className="border-r text-right">{moneyFormat(totalSales)}</TableCell>
...
export default ExportRevenuePreviewDialog
