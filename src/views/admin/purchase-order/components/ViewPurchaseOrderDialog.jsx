              ...
              <div className={cn(
                ...
                <div className={cn("space-y-6", !isDesktop && "space-y-4")}>
                  ...
                  <div className={cn(
                    ...
                    {purchaseOrder.purchaseContract?.status === 'liquidated' && purchaseOrder.purchaseContract?.liquidationValue != null && (
                      <div className="border border-orange-200 bg-orange-50/50 rounded-lg p-4 text-sm mt-4 col-span-1 md:col-span-2">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium text-orange-700">
                              {dateFormat(purchaseOrder.purchaseContract.liquidationDate, true)}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <strong>Giá trị bán lại:</strong>
                            <span className="font-bold text-orange-700">
                              {moneyFormat(purchaseOrder.purchaseContract.liquidationValue)}
                            </span>
                          </div>
                          {(() => {
                            const liquidationRemainingAmount = Number(purchaseOrder.purchaseContract.liquidationValue) - Number(purchaseOrder.paidAmount || 0);
                            return (
                              <div className="flex justify-between pt-2 border-t border-orange-200/60">
                                <strong>{liquidationRemainingAmount < 0 ? 'NCC cần hoàn tiền:' : 'Còn nợ NCC:'}</strong>
                                <span className={"font-bold text-base " + (liquidationRemainingAmount < 0 ? 'text-green-600' : 'text-red-600')}>
                                  {moneyFormat(Math.abs(liquidationRemainingAmount))}
                                </span>
                              </div>
                            );
                          })()}
                          ...
                          {(() => {
                            const liquidatedItems = (purchaseOrder.items || []).filter(
                              item => parseFloat(item.liquidationQuantity) > 0
                            )
