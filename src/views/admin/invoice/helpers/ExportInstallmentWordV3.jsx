...
/**
 * Export installment contract to Word document using template V3
 * @param {Object} data - Contract data
 * @param {string} filename - Output filename (default: hop_dong_ban_hang.docx)
 */
export async function exportInstallmentWordV3(data, filename = 'hop_dong_ban_hang_v3.docx') {
  try {
    // 1. Load template file
    const templatePath = '/templates/HopDongBan-Mau3.docx'
...
function formatMoney(n) {
  ...
  if (!n) return '0 Đ'
  // Try parsing out existing formatting if it's already a string but mostly numbers
  const parsed = parseFloat(String(n).replace(/[^\d.-]/g, ''))
  if (!isNaN(parsed)) return parsed.toLocaleString('vi-VN') + ' Đ'
  return n
}
