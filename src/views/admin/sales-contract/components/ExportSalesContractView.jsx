...
const statusLabel = (status) => {
  switch (status) {
    ...
    case 'confirmed': return 'Chờ giao hàng'
...
const ExportSalesContractView = ({
  ...
  const handleExport = async () => {
    ...
    const worksheet = workbook.addWorksheet('Hợp đồng bán', {
      views: [{ showGridLines: true }],
    })
    const table = document.getElementById('exportSalesContractTable')
    ...
    worksheet.getCell('A2').value = exportDate
      ? `Báo cáo danh sách hợp đồng bán - ${exportDate}`
      : 'Báo cáo danh sách hợp đồng bán'
    ...
    // Canh trái: B=2, C=3, D=4, E=5, F=6, S=19
    const leftAlignCols = [2, 3, 4, 5, 6, 19]
    ...
    // Chuyển thành số: Tổng tiền(9), Thuế(10), GG(11), Đã TT(12), Công nợ(13), Giá trị BL(16), Còn lại(18)
    const numberCols = [9, 10, 11, 12, 13, 16, 18]
    ...
    a.href = url
    a.download = `Báo cáo hợp đồng bán.xlsx`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="md:h-auto md:max-w-full">
        <DialogHeader>
          <DialogTitle>Danh sách hợp đồng bán</DialogTitle>
        </DialogHeader>

        <div className="max-h-[65vh] overflow-auto md:max-h-[75vh]">
          <div className="overflow-x-auto rounded-lg border">
            <Table className="min-w-full" id="exportSalesContractTable">
              <TableHeader>
                <TableRow className="bg-secondary text-xs">
                  ...
                  <TableHead className="min-w-28">Ngày giao</TableHead>
                  ...
                  <TableHead className="min-w-28">Giá trị BL</TableHead>
                  <TableHead className="min-w-28">Ngày BL</TableHead>
                  <TableHead className="min-w-28">Còn lại sau BL</TableHead>
                ...
                {data.map((contract) => {
                  ...
                  return (
                    <TableRow key={contract.id}>
                      ...
                      <TableCell>{contract.buyerName || contract.customer?.name}</TableCell>
                      <TableCell>{contract.buyerPhone || contract.customer?.phone}</TableCell>
                      <TableCell>{contract.buyerAddress || contract.customer?.address}</TableCell>
                      <TableCell>{contract.createdByUser?.fullName ?? '—'}</TableCell>
                      <TableCell>{dateFormat(contract.contractDate, false)}</TableCell>
                      <TableCell>{contract.deliveryDate ? dateFormat(contract.deliveryDate, false) : '—'}</TableCell>
                      ...
                      <TableCell>{paymentStatusLabel(contract.paymentStatus)}</TableCell>
                      <TableCell>{statusLabel(contract.status)}</TableCell>
                      <TableCell>{contract.liquidationValue ? moneyFormat(contract.liquidationValue, false) : '—'}</TableCell>
                      <TableCell>{contract.liquidationDate ? dateFormat(contract.liquidationDate, false) : '—'}</TableCell>
                      <TableCell>{contract.liquidationRemainingAmount ? moneyFormat(contract.liquidationRemainingAmount, false) : '—'}</TableCell>
...
export default ExportSalesContractView
